...
import { AdminCard, AdminInput, AdminPageHeader, AdminSectionCard, AdminSubmitButton } from "@/components/admin-ui";
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)]">
          <div className="space-y-6">
            <AdminSectionCard title="Add skill" description="Create a new skill badge and place it in the list order.">
              <form id="skills-form" action={createSkill} className="space-y-4">
                ...
                <AdminSaveBar formId="skills-form" label="Add Skill" pendingLabel="Adding..." helper="Saving updates the public skills page and revalidates cached content." />
              </form>
            </AdminSectionCard>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="flex items-center gap-3">
                        <img
                          src={item.icon}
                          alt={`${item.name} icon`}
                          className="h-10 w-10 rounded-full border border-black/5 object-contain p-1"
                        />
                        <div>
          ...
          <div className="space-y-6">
            <AdminCard>
              <div className="mb-5 border-b border-black/5 pb-4">
                ...
                <h2 className="mt-2 text-2xl font-bold tracking-tight text-[var(--foreground)]">Live skill snapshot</h2>
              </div>

              <div className="grid gap-3">
                {skills.slice(0, 8).map((item) => (
                  <div key={item.id} className="flex items-center gap-3 rounded-[1.35rem] border border-black/5 bg-[var(--surface)] p-4">
                    <img src={item.icon} alt="" className="h-10 w-10 rounded-full border border-black/5 object-contain p-1" />
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-[var(--foreground)]">{item.name}</p>
                      <p className="text-sm leading-6 text-[var(--muted)]">{item.category}</p>
                    </div>
                  </div>
                ))}
              </div>
